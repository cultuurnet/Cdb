...
class CultureFeed_Cdb_Data_Calendar_BookingPeriod implements CultureFeed_Cdb_IElement {
  ...
  public function setDateFrom($dateFrom) {
    ...
    if (!is_numeric($dateFrom)) {
      throw new UnexpectedValueException('Invalid from date: ' . $dateFrom . ', value should be a timestamp');
    }
  ...
  public function setDateTill($dateTill) {
    ...
    if (!is_numeric($dateTill)) {
      throw new UnexpectedValueException('Invalid from date: ' . $dateTill . ', value should be a timestamp');
    }
  ...
  public static function parseFromCdbXml(SimpleXMLElement $xmlElement) {
    ...
    if (empty($xmlElement->datefrom)) {
      throw new CultureFeed_Cdb_ParseException("Required attribute 'datefrom' is missing on bookingperiod");
    }

    if (empty($xmlElement->dateto)) {
      throw new CultureFeed_Cdb_ParseException("Required attribute 'dateto' is missing on bookingperiod");
    }

    $dateFrom = strtotime((string)$xmlElement->datefrom);
    $dateTill = strtotime((string)$xmlElement->dateto);

    return new CultureFeed_Cdb_Data_Calendar_BookingPeriod($dateFrom, $dateTill);
